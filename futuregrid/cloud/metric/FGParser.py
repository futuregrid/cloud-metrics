...
def read_from_sqlite3_and_convert_to_fg():

    import sqlite3 as lite

    instances = Instances()

#    instances.set_conf("futuregrid.local.cfg")
    future = instances.in_the_future
    past = instances.in_the_past

    try: 
        con = lite.connect('/home/metric/nimbus/sierra/sierra', detect_types = lite.PARSE_COLNAMES)
        con.row_factory = lite.Row
        con.text_factory = str
        cur = con.cursor()
        cur.execute('select t1.time as "t_start [timestamp]", t3.time as "t_end [timestamp]", t1.uuid as instanceId, t2.dn, t1.cpu_count as ccvm_cores, t1.memory as ccvm_mem, t1.vmm as serviceTag from create_events t1, user t2, remove_events t3 on t1.user_id=t2.id and t1.uuid=t3.uuid')
        rows = cur.fetchall()
        for row in rows:
            record = {}
            ...
            record["platform"] = "nimbus" # Need to be changed
            record["euca_hostname"] = "sierra" # need to be changed
            record["euca_version"] = "2.9" # need to be changed
            record["state"] = "Teardown" # need to be changed
            instances.eucadb.write(record)
